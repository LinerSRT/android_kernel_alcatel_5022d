...
#ifdef S5K4H5YC_USE_AWB_OTP
extern bool Daling_SK4H5_apply_OTP();
extern void Daling_SK4H5_read_OTP();
bool my_otp_flag1=0 ;
#endif
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
			if (*sensor_id == imgsensor_info.sensor_id) {				
				printk("it is dalin camera i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
...
static kal_uint32 open(void)
{
	...
	//while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
			if (sensor_id == imgsensor_info.sensor_id) {				
				printk("open it is dalin camera i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);	  
				break;
			}	
			LOG_INF("Read sensor id fail, id: 0x%x,0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
...
#if 0//def S5K4H5YC_USE_AWB_OTP
	if(my_otp_flag1==0)
	{
		printk("**********Daling_SK4H5_read_OTP********\n"); 	
		Daling_SK4H5_read_OTP();
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if(my_otp_flag1==0)
	{
		otp_ok = Daling_SK4H5_apply_OTP();
		if(otp_ok)
			my_otp_flag1 =1;
		printk("**********Daling_SK4H5_apply_OTP********my_otp_flag1 =%x,otp_ok = %x\n",my_otp_flag1,otp_ok); 	
	}
#endif 
