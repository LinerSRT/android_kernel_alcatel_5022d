...
#define PFX "S5K4H6YC_camera_sensor"
...
#define S5K4H6YC_USE_AWB_OTP
#ifdef S5K4H6YC_USE_AWB_OTP
	...
	//while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
			if (*sensor_id == S5K4H5YC_SENSOR_ID) {		
				*sensor_id = S5K4H6YC_SENSOR_ID;
				printk("it is sijichun camera i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	
				return ERROR_NONE;
			}	
...
static kal_uint32 open(void)
{
	...
	//while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			if (sensor_id == S5K4H5YC_SENSOR_ID) {	
				sensor_id = S5K4H6YC_SENSOR_ID;
				printk("it is sijichun cmaera i2c write id: 0x%x, sensor id:0x%x\n", imgsensor.i2c_write_id,sensor_id);	  
				break;
			}	
			LOG_INF("Read sensor id fail, id: 0x%x,0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	preview_setting();
	  ...
	  if(my_otp_flag==0)
		{
		 	write_cmos_sensor(0x3300,0x00);//enable tsf 0x00:enable default 0x01
		 	
			write_cmos_sensor(0x0100,0x01);
			mdelay(50);
			printk("**********S5K4H6YC enter otp update********\n"); 	
			otp_ok=otp_update();
			if(otp_ok)
			my_otp_flag=1;
			write_cmos_sensor(0x0100,0x00);
...
UINT32 S5K4H6YC_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	S5K4H6YC_MIPI_RAW_SensorInit	*/
