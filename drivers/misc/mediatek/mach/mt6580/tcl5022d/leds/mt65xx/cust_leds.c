...
static int disp_kpled(int level)
{
	static bool first_open=true;
	if(first_open)
	{
	mt_set_gpio_mode(GPIO_CAMERA_KEY1_PIN, GPIO_MODE_00);// GPIO MODE	
	mt_set_gpio_dir(GPIO_CAMERA_KEY1_PIN, GPIO_DIR_OUT);
	first_open = false;
	}
       if(level)
	   	mt_set_gpio_out(GPIO_CAMERA_KEY1_PIN, GPIO_OUT_ONE);
	 else
	 	mt_set_gpio_out(GPIO_CAMERA_KEY1_PIN, GPIO_OUT_ZERO);
}
/*
...
static struct cust_mt65xx_led cust_led_list[MT65XX_LED_TYPE_TOTAL] = {
	...
	{"button-backlight",  MT65XX_LED_MODE_GPIO, (int)disp_kpled,{0}},
