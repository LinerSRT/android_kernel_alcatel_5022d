...
u32 pinSet[3][8] = {
    ...
    else if(currSensorName && ((0 == strcmp(SENSOR_DRVNAME_S5K4H5YC_MIPI_RAW, currSensorName))||(0 == strcmp(SENSOR_DRVNAME_S5K4H6YC_MIPI_RAW, currSensorName))))
	{
	   if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMPDN],0)){PK_DBG("[CAMERA LENS] set gpio failed!! \n");}
	   if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMRST],0)){PK_DBG("[CAMERA SENSOR] set gpio failed!! \n");} //low == reset sensor
			mdelay(10);
...
//AF_VCC
          ...
          if(pinSetIdx == 0 || pinSetIdx == 2) {//disable sub
              if (GPIO_CAMERA_INVALID != pinSet[1][IDX_PS_CMRST]) {
                  ...
                  if(mt_set_gpio_dir(pinSet[1][IDX_PS_CMRST],GPIO_DIR_OUT)){PK_DBG("[CAMERA SENSOR] set gpio dir failed!! \n");}
                  if(mt_set_gpio_dir(pinSet[1][IDX_PS_CMPDN],GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! \n");}
                  if(mt_set_gpio_out(pinSet[1][IDX_PS_CMRST],pinSet[1][IDX_PS_CMRST+IDX_PS_ON])){PK_DBG("[CAMERA SENSOR] set gpio failed!! \n");} //low == reset sensor
		...
		msleep(5);
					...
					PK_DBG( "TURN ON 0310! end @%d \n", __LINE__ );
	}

	  else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_GC0311_MIPI_YUV, currSensorName)))
        {

				PK_ERR( "TURN ON gc0311! start! @%d \n", __LINE__ );
 ...
 //VCAM_A
	...
	{
	   if(mt_set_gpio_out(pinSet[0][IDX_PS_CMPDN],0)){PK_DBG("[CAMERA LENS] set gpio failed!! \n");}
	   if(mt_set_gpio_out(pinSet[0][IDX_PS_CMRST],0)){PK_DBG("[CAMERA SENSOR] set gpio failed!! \n");} //low == reset sensor
	   ...
	   if(TRUE != _hwPowerDown(SUB_CAMERA_POWER_VCAM_D,&regVCAMD_SUB))
		...
		if(mt_set_gpio_out(pinSet[1][IDX_PS_CMPDN],0)){PK_DBG("[CAMERA LENS] set gpio failed!! \n");}
		if(mt_set_gpio_out(pinSet[1][IDX_PS_CMRST],0)){PK_DBG("[CAMERA SENSOR] set gpio failed!! \n");} //low == reset sensor
	}
	else if(currSensorName && ((0 == strcmp(SENSOR_DRVNAME_S5K4H5YC_MIPI_RAW,currSensorName))||(0 == strcmp(SENSOR_DRVNAME_S5K4H6YC_MIPI_RAW, currSensorName))))
	{
		if(mt_set_gpio_out(pinSet[0][IDX_PS_CMPDN],0)){PK_DBG("[CAMERA LENS] set gpio failed!! \n");}
