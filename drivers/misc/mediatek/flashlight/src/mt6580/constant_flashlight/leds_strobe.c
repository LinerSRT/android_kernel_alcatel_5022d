...
#define FLASH_GPIO_MODE GPIO_CAMERA_FLASH_MODE_PIN  //flash mode pin 
#define FLASH_GPIO_EN GPIO_CAMERA_FLASH_EN_PIN  //movie mode enable pin

//#define FLASH_GPIO_MODE (GPIO1 | 0x80000000)  //flash mode pin 
//#define FLASH_GPIO_EN (GPIO3 | 0x80000000)  //movie mode enable pin
...
//	char bufR[2];
  ...
  else
  {
	...
	if(g_duty == 0 ) //torch mode
	{
	   ...
	   mt_set_gpio_out(FLASH_GPIO_MODE,GPIO_OUT_ZERO); 
	   printk("flash_led:torch mode.\n");
	}
	else	   //flash mode  
	{
		mt_set_gpio_out(FLASH_GPIO_EN,GPIO_OUT_ONE);
		mt_set_gpio_out(FLASH_GPIO_MODE,GPIO_OUT_ONE); 
...
int FL_Init(void)
{
  ...
  buf[0]=0xa;
	...
	if(mt_set_gpio_mode(FLASH_GPIO_MODE,GPIO_MODE_00)){PK_DBG("[constant_flashlight] set gpio mode failed!! \n");}
	if(mt_set_gpio_dir(FLASH_GPIO_MODE,GPIO_DIR_OUT)){PK_DBG("[constant_flashlight] set gpio dir failed!! \n");}
	if(mt_set_gpio_out(FLASH_GPIO_MODE,GPIO_OUT_ZERO)){PK_DBG("[constant_flashlight] set gpio failed!! \n");} 	
