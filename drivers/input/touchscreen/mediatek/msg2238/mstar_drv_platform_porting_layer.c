...
s32 DrvPlatformLyrTouchDevicePowerOn(void)
{
...
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)
	{
		...
		if (ret)
		{
			printk("msg2238 tpd_probe regulator_enable() failed!\n");
		}
#else
...
#if defined(CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM) || defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
    nRetVal = gpio_free(MS_TS_MSG_IC_GPIO_RST);     
    if (nRetVal < 0)
    {
        DBG("*** Failed to free GPIO %d, error %d ***\n", MS_TS_MSG_IC_GPIO_RST, nRetVal);
    }

    nRetVal = gpio_free(MS_TS_MSG_IC_GPIO_INT);
    if (nRetVal < 0)
    {
